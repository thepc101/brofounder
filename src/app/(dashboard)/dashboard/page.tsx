  ...
  return (
    <div className="flex h-full flex-col">
        ...
        actions={
          !project ? (
            <Link href="/onboarding">
              <Button size="sm" className="h-7 gap-1.5 rounded-lg bg-blue-500/15 text-[11px] text-blue-400/60 hover:bg-blue-500/20">
      ...
      <motion.div variants={stagger} initial="hidden" animate="visible"
        ...
        <motion.div variants={fadeIn} className="grid grid-cols-1 gap-3 lg:grid-cols-3">
          <div className="space-y-3 lg:col-span-2"><Recommendations /><ActivityFeed /></div>
          <div className="space-y-3"><TasksToday /></div>
        </motion.div>
        <motion.div variants={fadeIn} className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          <ValidationScore /><MarketOpportunity />
        </motion.div>
      </motion.div>
    </div>
  );
